...
var messageLedDevicePubHandler mqtt.MessageHandler = func(client mqtt.Client, msg mqtt.Message) {
    ...
    fmt.Printf("Received message: [%s] from topic: %s\n", deviceMsg, msg.Topic())
    handleDeviceMessageFromTopicMQTT(msg.Topic(), deviceMsg)
}
...
func botHandle(groupID string, userMsg string) {
    ...
    switch checkGroupID {
    case true:
        if userMsg == "delete_bot" {
            delete(groupIDBotStatusMap, groupID)
            fmt.Println("[Deleted bot]")
        }else {
...
func cmdListMapInit(controlDeviceArr []DeviceControlCode,
    ...
    for _, controlDevice :=range controlDeviceArr {
        controlDevice.ChatResponseMap["Timeout"] =  cfg.CmdConfig.DefaultRespMsg[msgTimeout]
    }
...
func handleBlockStatusOfBot(groupID string, userMsg string) {
    ...
    if len(splitUserMsg) == 1 {
        ...
        _, checkCmdVN := cmdListMapVN[userMsg]
        _, checkCmdEN := cmdListMapEN[userMsg]
        if checkCmdVN == true || checkCmdEN == true {
            sendToTelegram(groupID, "The bot is temporarily down, try: start or /start to get the bot working again")
        }
    }
}
...
func handleUserNeverEnteredPasswordBot(groupID string, userMsg string) {
    ...
    if len(splitUserMsg) == 1 {
        if splitUserMsg[0] == "start" || splitUserMsg[0] == "/start"{
            textLine1 := "I can help you control the devices in your home."
            textLine2 := "\n\nYour group has not entered a password for the bot. Enter the password in the format: \nstart + password or /start + password"
            textLine3 := "\n\nIf you do not know the password, please contact the bot creator."
            sendToTelegram(groupID, textLine1 + textLine2 + textLine3)
        }
    }else if len(splitUserMsg) == 2 {
        if splitUserMsg[0] == "/start" || splitUserMsg[0] == "start" {
            if splitUserMsg[1] == cfg.CmdConfig.BotPassword {
                ...
                cmdLine1 := "\nstart or /start - Bot working again"
                cmdLine2 := "\nclose or /close - Bot is temporarily down"
                sendToTelegram(groupID, "Enter password successfully. Bot is working\n" + cmdLine1 + cmdLine2)
                sendHelpResponseToTelegramUser(groupID, "You can try: ")
            }else {
                sendToTelegram(groupID, "Password input failed. Please try again")
            }
        }
...
func handleRunningStatusOfBot(groupID string, userMsg string) {
    ...
    if len(splitUserMsg) == 1 {
        if splitUserMsg[0] == "/close" || splitUserMsg[0] == "close" {
            ...
            sendToTelegram(groupID, "The bot is temporarily down, try: start or /start to get the bot working again")
        }else if splitUserMsg[0] == "/start" || splitUserMsg[0] == "start"{
            sendHelpResponseToTelegramUser(groupID, "You can try: ")
        }else {
...
func handleTeleCmd(groupID string, cmd string) {
    ...
    case Different:
        ...
        textKeyboard := textVN + "\n" + textEN
        sendHelpResponseToTelegramUser(groupID, textKeyboard)
    case AlmostSame:
        sendSuggestResponseToTelegramUser(groupID, chatCmdArr)
    case Same:
        scriptVN, checkCmdVN := cmdListMapVN[chatCmdArr[0].Data]
...
func sendHelpResponseToTelegramUser(groupID string, textKeyboard string) {
...
func sendSuggestResponseToTelegramUser (groupID string, chatCmdArr []StringCompare) {
    ...
    _, checkCmdVN := cmdListMapVN[chatCmd]
    if checkCmdVN == true {
