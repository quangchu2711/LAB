...
func handleUpdateMessage(updateMsg *tgbotapi.Message) {
    fmt.Printf("Type:[%T]\n", updateMsg)
    if updateMsg.NewChatMembers != nil {
        newChatMember := updateMsg.NewChatMembers[0]
        if newChatMember.UserName == cfg.TelegramConfig.UserName  {
            sendToBot(updateMsg.Chat.ID, "start")
            fmt.Println("[" + updateMsg.NewChatMembers[0].UserName + "]")
            fmt.Println("[Add quangcx_bot]")
        }
        fmt.Println(updateMsg.NewChatMembers)
    }else if updateMsg.LeftChatMember != nil{
        leftChatMember := updateMsg.LeftChatMember
        if leftChatMember.UserName == cfg.TelegramConfig.UserName {
            sendToBot(updateMsg.Chat.ID, "delete_bot")
            fmt.Println("[Delete quangcx_bot]")
        }
        fmt.Println(updateMsg.LeftChatMember)
    }else {
        sendToBot(updateMsg.Chat.ID, updateMsg.Text)
    }
}
